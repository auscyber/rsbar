...
return function()
	...
	local chevron = rsbar.add(
		...
		defaults.with_defaults({
			...
			padding_left = 0,
			padding_right = 0,
			-- Custom events are real in rsbar (`Kind::Custom`): this fires
			-- the same one items/menus.lua subscribes to.
			click_script = "rsbar trigger swap_menus_and_spaces",
		})
	)

	-- TODO(rsbar): `ItemPatch.label` has no `drawing` flag distinct from the
	-- item's own `drawing` -- `label = { drawing = false }` above only
	-- affects rsbar in that no `label` text was ever set (an unset label
	-- already renders as nothing), it does not toggle visibility
	-- independently of the icon the way SketchyBar's does.

	local front_app = rsbar.add(
		"front_app",
		"left",
		defaults.with_defaults({
			icon = { drawing = false },
	...
	-- comes from the `wm` module's own event. rsbar already has a native
