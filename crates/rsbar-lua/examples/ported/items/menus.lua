...
-- (helpers/menus/bin/menus, unrelated to SketchyBar or rsbar) and reading
-- its stdout back with SketchyBar's `sbar.exec(cmd, callback)`.
--
-- TODO(rsbar): rsbar-lua has no `sbar.exec`-equivalent at all -- `script`
...
--
-- TODO(rsbar): the original also groups `/menu\..*/` under one shared
...
return function()
	local apple_menu = rsbar.add("apple_menu", "left", {
		icon = { text = "" },
		label = { drawing = false },
	...
	local menu_placeholder = rsbar.add("menu.1", "left", {
		drawing = false,
		label = { text = "menu" },
	})

	local swap_watcher = rsbar.add("menu_watcher", "left", { drawing = false })
