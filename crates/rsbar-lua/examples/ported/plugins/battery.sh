...
# Ported from sketchybar/plugins/battery.sh, driving rsbar's own CLI instead
# of sketchybar's, and reading $RSBAR_NAME instead of $NAME.
#
# TODO(rsbar): there is no native battery-percentage source/event (see
...
#
# TODO(rsbar): the original also toggles `label.drawing` on `mouse.entered`/
...
rsbar item set "$RSBAR_NAME" --icon "$ICON" --label "${PERCENTAGE}%"
