...
//! ([`rsbar_lua::direct`]) asks the daemon for the same things, in the same
...
use rsbar_lua::require::Modules;
use rsbar_lua::{Channel, host};
...
rsbar.begin_config()
rsbar.bar({ height = 32, color = palette.bar, position = "top" })
rsbar.default({ icon = { font = "Hack Nerd Font:Bold:14.0" }, label = { padding_left = 4 } })

rsbar.add("event", "volume_change", "com.apple.sound.volumeChanged")

local clock = rsbar.add("item", "clock", { position = "right", update_freq = 10 })
...
local front = rsbar.add("item", "front_app", { position = "left" })
front:set({ label = { string = "Finder" } })

rsbar.add("bracket", { "clock", "/space%..*/" }, { background = { corner_radius = 6 } })

rsbar.set("front_app", { icon = { drawing = true } })
rsbar.move("clock", "after", "front_app")
rsbar.reorder({ "front_app", "clock" })
rsbar.trigger("volume_change", { INFO = "50" })
rsbar.update_all()
rsbar.end_config()
"#;
...
impl Scratch {
    fn new(name: &str) -> Self {
        let dir =
            std::env::temp_dir().join(format!("rsbar-lua-parity-{name}-{}", std::process::id()));
...
fn answer(request: &Request) -> Response {
    match request {
        Request::Query(rsbar_protocol::Query::Items) => Response::Items(Vec::new()),
        _ => Response::Ok,
    }
}
...
fn every_request_survives_the_wire_unchanged_so_skipping_it_loses_nothing() {
    ...
    for request in requests {
        let bytes = rsbar_protocol::wire::MessagePack
            .encode(&request)
            .expect("a request encodes");
        let decoded: Request = rsbar_protocol::wire::MessagePack
            .decode(&bytes)
            .expect("and decodes");
...
/// A config that calls `rsbar.run()` never returns on its own — it is an
...
fn while_running(
    ...
    loop {
        ...
        if !satisfied && pump(&channel, &seen) {
            satisfied = true;
            // Ends `rsbar.run()`, so the thread finishes rather than being
...
fn a_subscription_is_opened_with_the_same_request_either_way() {
    ...
    let script = scratch.write(
        "rc.lua",
        r#"
        local clock = rsbar.add("item", "clock", {})
...
fn events_pushed_with_no_port_reach_the_callback() {
    ...
    let script = scratch.write(
        ...
        clock:subscribe("system_woke", function(event)
            ...
            rsbar.trigger("mouse.entered", nil)
        end)
        rsbar.run()
        "#,
    );
    ...
    let seen = while_running(&script, |channel, _| {
        if pushed < 3 && channel.is_subscribed(&item) {
            assert!(
                channel.deliver(
                    &item,
                    &rsbar_protocol::Event::SystemWoke(rsbar_protocol::event::SystemWoke {})
                ),
