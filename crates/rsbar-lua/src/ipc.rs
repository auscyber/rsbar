...
pub struct IpcDispatcher {
    ...
    /// dispatcher is what `require("rsbar")` does, and a host that only ever
    /// reads `rsbar._VERSION` should not cost a thread.
    ///
...
fn subscribe(service: &str, request: &Request) -> Result<Events> {
    ...
    // to answer the next `rsbar.set`.
    let spawned = std::thread::Builder::new()
        .name("rsbar-lua-events".into())
        .spawn(move || {
            loop {
...
mod tests {
    ...
    /// only reads `rsbar._VERSION` should not cost a thread.
    #[test]
    fn no_thread_starts_until_the_first_request() {
        let dispatcher = IpcDispatcher::with_service("com.auscyber.rsbar.test.unused");
        assert!(
