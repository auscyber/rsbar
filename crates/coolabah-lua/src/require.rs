//! A `require` that lets a module's own top level call an async `coolabah`
...
//! whole script runs as one Lua coroutine, and a call like `coolabah.bar({...})`
...
pub fn install(lua: &Lua) -> mlua::Result<()> {
    lua.load(SHIM).set_name("=(coolabah require shim)").exec()
}
...
function require(name)
    ...
    for index = 1, #searchers do
        ...
        if type(loader) == "function" then
            ...
            -- this is what lets the chunk yield through an async `coolabah` call
