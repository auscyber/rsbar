//! Builds the `coolabah` table. Host-agnostic and entirely `async`: every verb
...
//! the daemon.
//!
//! A plain Lua script calling `coolabah.bar({...})` does not need to know any of
...
/// One item, as handed back by `coolabah.add`. Cheap to hold: cloning just
/// bumps the two `Arc`s.
#[derive(Clone)]
struct Item {
    name: ItemName,
...
impl UserData for Item {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        ...
        // channel waits for `coolabah.run()`.
        methods.add_method(
            "subscribe",
...
fn unexpected(response: &Response) -> mlua::Error {
    mlua::Error::RuntimeError(format!("coolabah: unexpected reply {response:?}"))
}

/// Everything `coolabah.add` needs to keep between calls. One value rather than
...
struct AddContext {
    dispatcher: Arc<dyn Dispatcher>,
    registry: Arc<Registry>,
    /// What `coolabah.default(...)` last stored — merged into every `add` from
...
/// Builds the `coolabah` table, wiring every verb to `dispatcher`.
///
/// # Errors
///
...
pub fn install(lua: &Lua, dispatcher: Arc<dyn Dispatcher>) -> mlua::Result<Table> {
    ...
    install_requests(lua, &coolabah, &dispatcher)?;

    coolabah.set("add", add_fn(lua, &context)?)?;
    coolabah.set("default", default_fn(lua, &context.defaults)?)?;
    coolabah.set("exec", exec_fn(lua)?)?;
    coolabah.set("subscribe", subscribe_fn(lua, &registry)?)?;
    coolabah.set("animate", animate_fn(lua)?)?;
    coolabah.set("delay", delay_fn(lua)?)?;
    coolabah.set("hotload", hotload_fn(lua)?)?;
    coolabah.set("set_bar_name", set_bar_name_fn(lua)?)?;
    ...
    coolabah.set("event_loop", event_loop.clone())?;
    coolabah.set("run", event_loop)?;

    coolabah.set("_VERSION", env!("CARGO_PKG_VERSION"))?;

    Ok(coolabah)
}

/// Every verb that ends in one or more requests, which is most of them: what
/// differs between `coolabah.remove` and `coolabah.reorder` is only which
...
fn install_requests(
    ...
    coolabah.set(
        "remove",
        verb!(|names: Value| {
            names_from_value(&names, "coolabah.remove")?
                .iter()
    ...
    // `coolabah.move("chevron", "after", "front_app")` — what the reference
    ...
    coolabah.set(
        "reorder",
        verb!(|names: Value| {
            let names = names_from_value(&names, "coolabah.reorder")?
                .iter()
    ...
    coolabah.set("update_all", verb!(|()| Ok(vec![Request::UpdateAll]))?)?;
    // `SbarLua` spells the same thing `update`; both are kept so neither a
    // config nor coolabah's own docs have to change.
    coolabah.set("update", verb!(|()| Ok(vec![Request::UpdateAll]))?)?;
    coolabah.set("reload", verb!(|()| Ok(vec![Request::Reload]))?)?;
    coolabah.set("shutdown", verb!(|()| Ok(vec![Request::Shutdown]))?)?;
    ...
    coolabah.set("begin_config", verb!(|()| Ok(vec![Request::BeginConfig]))?)?;
    coolabah.set("end_config", verb!(|()| Ok(vec![Request::EndConfig]))?)?;
...
/// `coolabah.trigger("demo", { VAR = "Test" })` — a built-in carries its own
/// payload; only a custom event takes variables.
fn event_from_lua(name: &str, data: Option<&Value>) -> mlua::Result<crate::protocol::Event> {
    let mut event = crate::convert::kind_from_str(name)?.into_event();
    if let (crate::protocol::Event::Custom(custom), Some(data)) = (&mut event, data) {
...
fn relative_from_str(text: &str) -> mlua::Result<Relative> {
    match text {
        ...
        other => Err(mlua::Error::RuntimeError(format!(
            "coolabah.move takes \"before\" or \"after\", got \"{other}\""
        ))),
    }
}
...
/// `opts` layered over whatever `coolabah.default` last stored, deep-merged so
...
impl AddContext {
    ...
    /// `coolabah.add(kind, name?, ...)` — see [`add_fn`] for the vocabulary.
    async fn add(
        &self,
        lua: &Lua,
        kind: &str,
        ...
        // positional argument coolabah has nowhere to put.
...
/// got skipped — for a kind coolabah does not model, whose options may not be
/// the third argument.
        ...
        (_, Value::Table(_)) => {
            if !matches!(first, Value::Nil) {
                tracing::error!(
                    %kind,
                    argument = ?first,
                    "ignoring an argument coolabah has nowhere to put for this add kind"
                );
            }
            second
        }
...
/// `coolabah.add(kind, name?, ...)` — `SketchyBar`'s own calling convention,
/// kept deliberately close to it rather than to a different shape coolabah might
...
/// `coolabah.default(opts)` — properties merged into every `add` from this point
...
/// `coolabah.exec(command, callback?)` — runs `command` through `sh -c` and, if
...
/// binary (`bin/coolabah_lua.rs`) `spawn_local`-ing it instead — worth doing if
    ...
    lua.create_async_function(
        move |_, (command, callback): (String, Option<Function>)| async move {
            match tokio::process::Command::new("sh")
                .arg("-c")
                .arg(&command)
                // A config shells out to `coolabah` — `--subscribe`, `--move`,
                ...
                Ok(output) => {
                    if !output.status.success() {
                        tracing::warn!(
                            ...
                            "coolabah.exec exited non-zero"
                        );
                    }
                    if let Some(callback) = callback {
                        let stdout = String::from_utf8_lossy(&output.stdout).into_owned();
                        if let Err(err) = callback.call_async::<()>(stdout).await {
                            tracing::error!(%command, %err, "an coolabah.exec callback failed");
                        }
                    }
                }
                Err(err) => {
                    tracing::error!(%command, %err, "coolabah.exec could not run the command")
                }
            }
            Ok(())
        },
...
/// `coolabah.subscribe(item, events, callback)` — the same registration
...
/// `coolabah.animate(curve, duration, function() ... end)` — runs the body
/// straight through. coolabah has no animation to hand the curve to, so the sets
    ...
    lua.create_async_function(
        move |_, (curve, duration, body): (String, u32, Function)| async move {
            tracing::warn!(
                %curve,
                duration,
                "coolabah cannot animate yet; applying this block's changes at once"
            );
...
/// `coolabah.delay(seconds, function() ... end)`.
...
/// `coolabah.hotload(true)` — accepted and ignored. The daemon watches its own
/// configured file either way, so there is nothing for a config to turn on.
fn hotload_fn(lua: &Lua) -> mlua::Result<Function> {
    lua.create_function(move |_, enabled: bool| {
        tracing::debug!(
            enabled,
            "coolabah reloads its own config; hotload is always on"
        );
        Ok(())
    })
}

/// `coolabah.set_bar_name(name)` — refused rather than silently talking to the
/// wrong daemon: which bar this connects to is `COOLABAH_SERVICE`, fixed when
/// the dispatcher was built.
fn set_bar_name_fn(lua: &Lua) -> mlua::Result<Function> {
    lua.create_function(move |_, name: String| {
        tracing::error!(
            %name,
            "coolabah picks its daemon from COOLABAH_SERVICE, not from the config; ignoring"
        );
        Ok(())
    })
}
...
mod tests {
    ...
    /// Runs `script` against a fresh `coolabah` table and hands back both the
    ...
    fn run(script: &str) -> (mlua::Result<()>, Vec<Request>) {
        ...
        lua.globals().set("coolabah", table).unwrap();
        // `enable_all`, so `coolabah.delay` has a timer to sleep on.
    ...
    fn adding_an_event_declares_one_the_config_fires_itself() {
        assert_eq!(
            requests(r#"coolabah.add("event", "demo")"#),
    ...
    fn adding_an_event_with_a_notification_bridges_it() {
        assert_eq!(
            requests(r#"coolabah.add("event", "demo", "com.example.thing")"#),
            vec![Request::AddEvent {
                name: "demo".parse().unwrap(),
    ...
    fn adding_a_built_in_event_is_an_error_rather_than_a_second_one() {
        let (result, requests) = run(r#"coolabah.add("event", "volume_changed")"#);
    ...
    fn assigning_a_property_sends_the_same_patch_a_set_would() {
        let through_metatable = requests(
            r#"local item = coolabah.add("item", "clock")
               item.label = "12:00""#,
        );
    ...
    fn a_popups_drawing_is_reachable_through_the_metatable() {
        ...
        let requests = requests(
            r#"local item = coolabah.add("item", "volume")
    ...
    fn a_namespace_keeps_accumulating_the_path() {
        ...
        assert_eq!(
            ...
            Some(crate::protocol::Color(0xffff_0000))
        );
        assert_eq!(
            patches[2]
                .geometry
    ...
    fn every_kind_reaches_the_daemon_as_its_own_component() {
        let added: Vec<ComponentKind> = requests(
            r#"coolabah.add("item", "an_item")
               coolabah.add("alias", "Control Centre,FocusModes")
               coolabah.add("bracket", "a_bracket", { "an_item" })
               coolabah.add("space", "a_space")
               coolabah.add("graph", "a_graph", 50)
               coolabah.add("slider", "a_slider", 100)"#,
        )
        .into_iter()
        .filter_map(|request| match request {
    ...
    fn an_alias_mirrors_the_menu_bar_item_it_is_named_after() {
        let requests = requests(r#"coolabah.add("alias", "Control Centre,FocusModes")"#);
        assert_eq!(
    ...
    fn an_explicit_alias_wins_over_the_items_own_name() {
        let requests =
            requests(r#"coolabah.add("alias", "focus", { alias = "Control Centre,FocusModes" })"#);
        assert_eq!(
            patches(&requests)[0].alias.as_deref(),
            Some("Control Centre,FocusModes")
        );
    ...
    fn an_unnamed_add_is_counted_per_kind() {
        let names: Vec<String> = requests(
            r#"coolabah.add("bracket", { "a" })
               coolabah.add("item", {})
               coolabah.add("bracket", { "b" })"#,
        )
        .iter()
        .filter_map(|request| match request {
    ...
    fn a_brackets_members_ride_on_the_patch_so_patterns_survive() {
        ...
        let requests = requests(r#"coolabah.add("bracket", { "/menu\\..*/" }, {})"#);
        assert_eq!(
    ...
    fn a_sliders_positional_width_becomes_the_items_width() {
        let requests = requests(
            r#"coolabah.add("slider", "volume_options", 100, { position = "popup.volume" })"#,
        );
        assert_eq!(
    ...
    fn a_popup_child_hangs_off_the_item_that_owns_the_popup() {
        let requests = requests(
            r#"coolabah.add("alias", "Fantastical,Fantastical", { position = "popup.overflow" })"#,
        );
        assert_eq!(
            requests[0],
    ...
    fn an_unknown_kind_is_still_added_as_a_plain_item() {
        let requests = requests(r#"coolabah.add("widget", "thing", { label = "hi" })"#);
        assert_eq!(
            requests[0],
    ...
    fn a_second_argument_that_is_neither_a_name_nor_the_kinds_own_is_refused() {
        let (result, requests) = run(r#"coolabah.add("bracket", 42, {})"#);
    ...
    fn defaults_are_merged_into_every_add() {
        let requests = requests(
            r#"coolabah.default({ icon = { color = 0xffff0000 } })
               coolabah.add("item", "clock", { label = "12:00" })"#,
        );
    ...
    fn a_graph_takes_samples_by_name_and_through_its_handle() {
        assert_eq!(
            requests(
                r#"local graph = coolabah.add("graph", "cpu", 50)
                   graph:push(0.5)
                   coolabah.push("cpu", 0.25)"#
            )
            .into_iter()
    ...
    fn moving_and_reordering_are_reachable_without_shelling_out() {
        assert_eq!(
            requests(
                r#"coolabah.move("chevron", "after", "front_app")
                   coolabah.reorder({ "chevron", "front_app" })"#
            ),
            vec![
                Request::Move {
                    name: name("chevron"),
                    relative: crate::protocol::Relative::After,
    ...
    fn set_by_name_also_takes_sketchybars_pattern_shorthand() {
        ...
        let requests = requests(r#"coolabah.set("/menu\\..*/", { drawing = false })"#);
        assert!(matches!(
            &requests[0],
    ...
    fn remove_takes_one_name_or_a_list_of_them() {
        assert_eq!(
            requests(
                r#"coolabah.remove("clock")
                   coolabah.remove({ "a", "/menu\\..*/" })"#
            ),
            vec![
    ...
    fn an_animated_block_still_applies_its_changes() {
        let requests = requests(
            r#"local item = coolabah.add("item", "clock")
               coolabah.animate("sin", 30, function()
                   item.label = "12:00"
               end)"#,
        );
        assert_eq!(
    ...
    fn a_delayed_block_runs_after_the_wait() {
        let requests = requests(
            r#"local item = coolabah.add("item", "clock")
               coolabah.delay(0, function() item.label = "later" end)"#,
        );
        assert_eq!(
            patches(&requests)
                .last()
    ...
    fn subscribing_by_name_reaches_the_same_registry_the_handle_does() {
        // Purely local until `coolabah.run()`, so the assertion is that neither
        // spelling raises and neither puts anything on the wire.
        assert_eq!(
            requests(
                r#"local item = coolabah.add("item", "clock")
                   coolabah.subscribe(item, "front_app_switched", function() end)
                   coolabah.subscribe("clock", { "space_change" }, function() end)"#
            )
            .into_iter()
