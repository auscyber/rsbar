...
//!
//! Everything `bin/coolabah_lua.rs` does to turn a file into a running config,
...
//!   VM, the `coolabah` table and the dispatcher behind it can live wherever the
...
//!   requests are answered by a pass on another thread; `coolabah.exec` runs a
//!   subprocess and `coolabah.delay` a timer, both on the runtime this thread
...
//!
//! `coolabah.exec` spawns a subprocess and `coolabah.delay` sleeps; both are tokio's
...
//! `coolabah-lua config.lua` run *the same code on the same runtime*, which is
...
/// A `Lua` state with the `coolabah` API in it, ready to run a config.
pub struct Host {
    pub lua: Lua,
...
pub fn build(
    ...
    let coolabah = crate::api::install(&lua, dispatcher)?;
    install_module(&lua, &coolabah)?;
...
/// for `SbarLua` says `sbar`; coolabah's own docs say `coolabah`. All three are the
/// same table, so a config only ever changes its `require` — which is the
/// whole point of the API.
fn install_module(lua: &Lua, coolabah: &mlua::Table) -> mlua::Result<()> {
    let globals = lua.globals();
    globals.set(name!(), coolabah.clone())?;
    globals.set("sbar", coolabah.clone())?;
    globals.set("sketchybar", coolabah.clone())?;

    let package: mlua::Table = globals.get("package")?;
    let loaded: mlua::Table = package.get("loaded")?;
    loaded.set(name!(), coolabah.clone())?;
    loaded.set("sbar", coolabah.clone())?;
    loaded.set("sketchybar", coolabah.clone())
}
...
fn run(dispatcher: &Arc<dyn Dispatcher>, script: &Path, modules: &Modules) -> Outcome {
    ...
    // The whole config is one coroutine, which is what lets `coolabah.bar(...)`
...
mod tests {
    ...
    use crate::protocol::{ItemName, Kind, Request, Response};
    use crate::require::Modules;
    ...
    impl Scratch {
        fn new(name: &str) -> Self {
            let dir = std::env::temp_dir()
                .join(format!("coolabah-lua-host-{name}-{}", std::process::id()));
    ...
    fn every_name_a_config_reaches_for_finds_the_same_table() {
        ...
        host.lua
            .load(
                r"
                assert(rawequal(coolabah, sbar), 'sbar is the coolabah table')
                assert(rawequal(coolabah, sketchybar), 'sketchybar is too')
                assert(rawequal(coolabah, require('sketchybar')), 'and so is the require')
                ",
            )
            .exec()
    ...
    fn a_config_runs_off_thread_and_reports_the_modules_it_loaded() {
        ...
        let script = scratch.write(
            ...
            coolabah.bar({ color = palette.bar })
            coolabah.add('item', 'clock', { position = 'right' })
            ",
        );
