//! Runs a `.lua` config against a running `coolabah`.
//!
//! Almost nothing happens here. [`coolabah_lua::host`] is what turns a file into
...
use coolabah_lua::{IpcDispatcher, host, require::Modules};

fn main() -> ExitCode {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_env(name!(env "LOG"))
                .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new("info")),
        )
        .init();

    let Some(path) = std::env::args().nth(1) else {
        eprintln!("usage: coolabah-lua <config.lua>");
    ...
    let dispatcher: Arc<dyn coolabah_lua::Dispatcher> = Arc::new(IpcDispatcher::new());
