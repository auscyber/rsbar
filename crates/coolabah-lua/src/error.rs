...
use crate::protocol::event::{InvalidEvent, InvalidEventName, InvalidNotificationName};
use crate::protocol::{InvalidName, InvalidPosition, Response};
...
pub enum ApiError {
    ...
    #[error("coolabah is not running")]
    NotRunning,
    #[error("lost contact with coolabah: {0}")]
    Transport(#[source] async_mach_ports::Error),
    #[error("coolabah rejected the request: {0}")]
    Rejected(String),
    #[error("coolabah answered with {0:?}, which does not belong to this request")]
...
impl From<ApiError> for mlua::Error {
    fn from(err: ApiError) -> Self {
        Self::RuntimeError(format!("coolabah: {err}"))
    }
}
