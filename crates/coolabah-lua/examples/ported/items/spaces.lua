...
--
-- coolabah already has native events for exactly this idea: `space_changed`
...
-- from a real event.
--
-- TODO(coolabah): there is no way to *discover* how many spaces/workspaces
-- exist (no `Query::Spaces` in crates/protocol/src/lib.rs), so the count
-- below is a guess, not read from anything live.
-- TODO(coolabah): there is also no request to *switch* to a space -- `Request`
...
-- CLI (unrelated to coolabah) from `click_script` instead.
...
return function()
	...
	for i = 1, space_count do
		spaces[i] = coolabah.add("space." .. i, "left", {
