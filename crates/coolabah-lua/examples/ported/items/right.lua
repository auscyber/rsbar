...
-- coolabah-lua's runner never sets up `package.path`/`arg` relative to the
...
local plugin_dir = "crates/coolabah-lua/examples/ported/plugins"
...
-- an ordinary item whose name has to be coolabah-legal (no comma/space:
...
-- TODO(coolabah): the original's negative `padding_left = -15` (used to tuck
-- SketchyBar's own alias rendering closer to its neighbour) has no
-- equivalent guarantee here -- coolabah accepts negative padding, but there is
-- nothing that promises alias rendering lines up with SketchyBar's the same
-- way, since coolabah is not API-compatible with it.

-- Clock: the original polls via plugins/clock.sh; coolabah's own per-item timer
...
-- event and $INFO. coolabah has a native `volume_changed` event carrying
-- `volume` (crates/protocol/src/event.rs) -- subscribed directly, no script.
local volume = coolabah.add("volume", "right", defaults.with_defaults({}))
...
-- TODO(coolabah): the original also drives a `sbar.add("slider", ...)` in a
...
-- coolabah's native `power_source_changed` event only carries an AC/BATTERY/
...
-- TODO(coolabah): no wattage figure is available to show even approximately.

-- Battery: no native battery-percentage source exists at all (see
-- crates/coolabah/src/sources -- there is brightness/volume/wifi/power/media/
...
-- popup, discovered via `sketchybar --query default_menu_items`. coolabah's
-- `coolabah.query.menu_items()` (Query::MenuItems in crates/protocol/src/lib.rs)
-- is the direct native equivalent for *discovering* them.
--
-- TODO(coolabah): there is no `popup` to put the discovered aliases into --
...
-- So this only reports the count coolabah can see, in the item other coolabah
-- items already use to summarise something the popup would have expanded.
local overflow = coolabah.add(
	"overflow",
	"right",
	defaults.with_defaults({
...
local ok, menu_items = pcall(function()
	return coolabah.query.menu_items()
end)
if ok then
