...
--
-- Returns a setup function rather than calling `coolabah.bar` at require time:
-- `coolabah.*` calls are async (they await the daemon over IPC), and LuaJIT
...
return function()
	coolabah.bar({
		-- SketchyBar's `position = "top"` is coolabah's `edge`.
		edge = "top",
		height = 40,
		color = colors.transparent,
		-- TODO(coolabah): `BarPatch` (crates/protocol/src/lib.rs) has no
		-- `display` field -- no way to say "span every display" vs. "just
		-- the main one".
		-- TODO(coolabah): `BarPatch` also has no bar-level `padding_right`/
