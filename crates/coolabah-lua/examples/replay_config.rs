...
//! `cargo run -p coolabah-lua --example replay_config -- ~/dendritic/sketchybar`
...
fn main() -> mlua::Result<()> {
    ...
    // `src/bin/coolabah_lua.rs`: an unsafe state so a config's own `require` can
