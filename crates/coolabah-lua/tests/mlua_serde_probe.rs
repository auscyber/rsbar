...
//! in either front end.
//!
//! If one of these ever fails, the fix is in `coolabah_protocol`, not here.

use coolabah_protocol::{BoolChange, Color, ItemPatch};
...
fn a_key_no_patch_has_a_field_for_costs_that_key_and_nothing_else() {
    ...
    // helper. coolabah carries only the name, so the table is dropped rather
