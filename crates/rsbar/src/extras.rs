...
impl Scanner {
    ...
    /// Starts a scan, unless one is already out.
    ///
    ...
    #[main_thread(pass)]
    pub(crate) fn request(&self, access: Trusted) {
    ...
    pub(crate) fn take(&self) -> bool {
        ...
        let Some(items) = held.take() else {
            return false;
        };
        // Cleared under the lock: a scan publishes by filling this slot and
        // then raising the flag, so clearing it outside would let the next
        // scan's flag be cleared before anyone saw it.
