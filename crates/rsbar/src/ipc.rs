...
use rsbar_protocol::wire::Receiver;
...
mod tests {
    ...
    use rsbar_protocol::{Request, Response};
    use std::cell::Cell;
    use std::rc::Rc;
    ...
    fn a_request_arrives_on_the_run_loop_and_can_be_answered() {
        ...
        let client = std::thread::spawn(move || {
            ...
            let sender =
                rsbar_protocol::wire::Sender::<Request>::connect(&client_name, MessagePack)
                    .expect("connect");
            sender
                .call_blocking::<Response>(&Request::Query(rsbar_protocol::Query::Bar))
                .expect("call")
        });
