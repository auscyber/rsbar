    ...
    fn drop(&mut self) {
        if let Some(watcher) = self.0.take() {
            ...
            if std::thread::Builder::new()
                .name("rsbar-config-stop".to_owned())
...
mod tests {
    ...
    fn an_atomic_rename_over_save_is_caught() {
        ...
        for generation in 1..=3u32 {
            let temp = dir.join(format!("rsbarrc.tmp{generation}"));
