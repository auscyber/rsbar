...
/// available as `RSBAR_AX_WIDE=1`, see [`WIDE_NOTIFICATIONS`]) and counting
...
///
/// Re-run the measurement rather than editing this by eye: `RSBAR_AX_WIDE=1`
/// with `RSBAR_LOG=rsbar=trace`, then count `notification="..."` in the log.
...
///
/// Behind `RSBAR_AX_WIDE=1` rather than always on, because the answer is
...
fn notification_names() -> &'static [&'static str] {
    if std::env::var_os("RSBAR_AX_WIDE").is_some() {
        WIDE_NOTIFICATIONS
    } else {
        NOTIFICATIONS
    }
...
fn create_on_sources(
    ...
    if context.watched.lock().unwrap().is_empty() {
        return Ok(());
    }
...
fn on_notification(notified: ax::Notified<'_, Arc<Context>, Option<Entity>>) {
    ...
    // repaint at all.
    let entities: Vec<Entity> = context.watched.lock().unwrap().keys().copied().collect();
    ...
    pub fn sync_one(
        ...
        match self.watching.entry(pid) {
            ...
            Entry::Occupied(slot) => {
                ...
                let moved = {
                    let mut watched = watching.context.watched.lock().unwrap();
