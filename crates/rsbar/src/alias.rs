...
//! `rsbar/examples/ax_observer_probe.rs`, macOS 26.5.1) and both ruled out:
//!
...
pub struct Alias {
    ...
    /// The same window, as the id a [`Snapshot`] is keyed by, so the cached
    /// one can be re-checked against a fresh list without a window server
...
impl Captures {
    ...
    fn ask_for_scan(&mut self, owner: &str, snapshot: &Snapshot) {
        ...
        match skylight::ax::Trusted::now() {
            ...
            // annotated, so the attribute supplies it.
            Some(access) => self.scan.request(mtm, access),
            ...
            // which is what a pre-macOS-26 machine reports anyway.
