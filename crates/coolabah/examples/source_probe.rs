...
//! `ecs::run` uses. If a source fires here, it fires in `coolabah`.
//!
//! ```text
//! cargo run -p coolabah --example source_probe -- brightness_changed
//! cargo run -p coolabah --example source_probe -- wifi_changed
//! cargo run -p coolabah --example source_probe -- space_changed
//! cargo run -p coolabah --example source_probe -- space_windows_changed
//! cargo run -p coolabah --example source_probe -- media_changed
//! RUST_LOG=coolabah=info cargo run -p coolabah --example source_probe -- power_source_changed
//! ```
//!
...
use coolabah::runloop::Waker;
use coolabah::sources::Registry;
use coolabah_protocol::Kind;
...
fn main() {
    ...
    let providers: Vec<coolabah::sources::SourceId> = registry.providers_of(&kind).to_vec();
