...
use coolabah::alias;
use coolabah::alias::{Alias, Captures, MenuBarItem, RawMenuBarWindow};
...
fn time_a_pass(items: &[MenuBarItem]) {
    ...
    let mut captures = Captures::new(coolabah::runloop::Waker::install(proof, || {}));
    ...
    let ids: Vec<_> = coolabah::alias::diagnostics(&snapshot)
        .into_iter()
