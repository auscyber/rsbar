...
//!
//! The `coolabah` client spends most of its time being a process, so a burst
...
//!
//! ```text
//! COOLABAH_SERVICE=... cargo run --release --example ipc_burst -- 500
//! ```

use async_mach_ports::SendPort as _;
use coolabah_protocol::wire::{MessagePack, Sender};
use coolabah_protocol::{Query, Request, Response, service_name};
...
fn main() -> std::process::ExitCode {
    ...
    let Ok(sender) = Sender::<Request>::connect(&service, MessagePack) else {
        eprintln!("coolabah is not running on {service}");
