...
use coolabah::bar::Settings;
use coolabah::components::{Label, Order, bundle};
use coolabah::ecs::{FrameBudget, close_frame, presenting_bar, remember_bar, remember_popups};
use coolabah::layout::{DirtyItems, ForceRepaint, clear_force_repaint, needs_repaint};
use coolabah::popup::needs_repaint_popups;
use coolabah_protocol::{ItemName, Position};
        ...
        if capped {
            schedule.add_systems(
                (
                    ...
                    clear_force_repaint.run_if(coolabah::ecs::presenting),
                    close_frame,
                )
                    .chain(),
            );
