...
//! |---|---|---|---|
//! | `coolabah-ax` ×16 | [`crate::alias::Captures`]'s scanner | [`build`] | wakes [`waker`], read on the next pass |
//! | `coolabah-capture` ×4 | [`crate::alias::Captures`]'s captor | [`build`] | the same |
//! | `coolabah-script-N` | [`Scripts`] | [`build`], from `main` | a finished job wakes [`waker`] |
//! | `coolabah-subscriber-N` | [`crate::subscribers::Subscribers`] | on subscribe | nothing comes back; it only sends |
...
use crate::protocol::event::{MouseEnter, MouseExit, SpaceChange};
use crate::protocol::{Event, Kind, PressTarget, Query as ProtocolQuery, Request, Response};
...
fn arm_wake(budget: Res<FrameBudget>, mut wake: NonSendMut<Wake>, main: NonSend<Main>) {
    match budget.deferred() {
        // `fired()`, not just `is_none()`: the registration is held to keep the
        // wake alive, and holding it does not end when it fires. A wake that
        // has already been delivered leaves a spent registration here, and its
        // task has run to completion -- so treating that as "still armed" is
        // what makes the bar stop waking until some unrelated pass comes
        // along. Re-arm instead.
        Some(left)
            if wake
                .0
                .as_ref()
                .is_none_or(crate::runloop::Registration::fired) =>
        {
...
/// The window server connection, exclusive for the length of one pass.
///
/// Inserted once at the start of [`pass`]'s frame body and removed at the
/// end — see that function — so every system in between that touches a
/// window borrows it from here rather than acquiring one of its own. `NonSend`
/// for the same reason as [`Main`]: everything that reads this also touches
/// `NonSend` state of its own (`Panels`, `Popups`, a `CGContext`), so there is
/// no thread this could usefully run a system on that those could not.
pub struct ConnectedRes(pub skylight::Connected);
...
pub struct IpcRequest {
    ...
    pub reply: Option<crate::protocol::wire::Reply>,
...
pub fn build(
    ...
    // The bar, as an entity, and the claims it holds on its own behalf.
    //
    // A claim belongs to an entity, and until now every entity was an item a
    // config asked for -- so a source the bar itself depends on only ran when
    // some item happened to want the same event. `display_change` is the older
    // example: `rebuild_panels` needs it to put the panels back after a display
    // arrives, and got it only by luck. `system_woke` is the one that showed
    // it: nothing claimed it, so nothing delivered it, so waking from sleep
    // reached the daemon not at all.
    let bar = app.world_mut().spawn(crate::components::Bar).id();
    let watching = crate::components::Watching(
        registry.watch_all(bar, [Kind::SystemWoke, Kind::DisplayChanged]),
    );
    app.world_mut().entity_mut(bar).insert(watching);
    ...
    app.add_systems(First, drain_events)
        ...
        .add_systems(
            ...
            (
                ...
                // A popup's own gate, its own retained placements -- and, per
                // `repaint_popups`'s own doc, the bar's gate too: an open
                // popup hangs off its host's on-screen frame, which moves
                // whenever the bar repaints even if nothing about the popup
                // itself changed.
                layout::needs_repaint.pipe(remember_popups),
...
/// Wakes the frame task — see [`pass`].
///
/// A single [`tokio::sync::Notify`] rather than a channel: it holds at most
/// one permit, so a burst of wakes before the task next runs coalesces into
/// the one pass that burst deserves, the same way `waker`'s one shared run
/// loop source already coalesces signals inside CoreFoundation.
static FRAME: tokio::sync::Notify = tokio::sync::Notify::const_new();

thread_local! {
    /// The frame task, polled directly by [`pass`] rather than through
    /// [`crate::runloop`]'s `LocalSet`.
    ///
    /// A `LocalSet` was the first shape this took, but `app.update()` runs
    /// inside this task's own poll, and a system anywhere in the schedule may
    /// call `runloop::spawn`/`owned` -- which needs that *same* `LocalSet`'s
    /// borrow to still be free. Sharing one would make the frame task's own
    /// poll reentrant against itself the moment any system did that, which is
    /// a deadlock `runloop`'s `RefCell`s are built to catch, not something to
    /// route around. A future of its own, in a `RefCell` of its own, has
    /// nothing for `runloop::spawn` to collide with.
    static FRAME_TASK: RefCell<Option<std::pin::Pin<Box<dyn std::future::Future<Output = ()>>>>> =
        const { RefCell::new(None) };
}

/// Builds the frame task, once.
///
/// Lazy, and idempotent, rather than started from [`run`]: a test parks an
/// app and calls [`pass`] directly without ever calling `run`, and the task
/// has to exist by the time it does.
///
/// `try_borrow`, not `borrow`: a Carbon handler dispatched from inside
/// `app.update()` can call [`pass`] reentrantly, which calls this while
/// [`poll_frame_task`] is still holding its own borrow across the poll that
/// led here. A borrow this cannot get is proof the task already exists —
/// nothing else is ever mid-construction of it — so there is nothing to do.
fn ensure_frame_task() {
    FRAME_TASK.with(|slot| {
        let Ok(borrowed) = slot.try_borrow() else {
            return;
        };
        if borrowed.is_some() {
            return;
        ...
        *slot = Some(Box::pin(async {
            loop {
                FRAME.notified().await;
                run_frame().await;
            }
        }));
    });
...
/// Polls the frame task once, using the same waker every other wake in this
/// module goes through.
///
/// So a suspension this poll leaves behind — the rare pass that lands while
/// the capture pool holds [`skylight::Connected`] — resumes exactly the way a
/// deadline or a request does: something calls [`crate::runloop::Waker::wake`]
/// on the shared source, the run loop takes a turn, and `pass` runs again,
/// polling the same still-pending future onward from where it left off.
fn poll_frame_task() {
    FRAME_TASK.with(|slot| {
        let Ok(mut slot) = slot.try_borrow_mut() else {
            // Re-entered from inside a pass -- a Carbon handler dispatched
            // while we were already updating. The outer poll will see
        ...
        let waker = pass_waker(pass_thread());
        let _ = task
            .as_mut()
...
/// Proof that this is the thread [`pass`] runs on.
///
/// `pass` is only ever called from the main thread in production -- a run
/// loop callout, or `run` itself. libtest runs a test on a thread of its own,
/// so a test asks [`crate::runloop::main_thread`] the same way `daemon()`
/// below already does for everything else that needs this proof.
fn pass_thread() -> objc2::MainThreadMarker {
    #[cfg(test)]
    {
    ...
    #[cfg(not(test))]
    {
        objc2::MainThreadMarker::new().expect("pass runs on the main thread")
    }
}

/// One frame's worth of window server work: the connection, held from here
/// to the end of the pass, and the schedule run underneath it.
///
/// Async only for the acquire; everything from the coalescing drain through
/// `app.update()` is the ordinary synchronous body a pass has always had, run
/// inside the connection's lock rather than around it — see
/// [`crate::ecs::ConnectedRes`].
async fn run_frame() {
    // A test never makes a `Window`, so nothing else would establish the
    // connection before this, the first thing that needs it, asks.
    let _ = skylight::establish(pass_thread());
    ...
    APP.with(move |slot| {
        ...
        app.world_mut().insert_non_send(ConnectedRes(connected));
        app.update();
        // Dropped, not merely removed: holding the guard one instant longer
        // than the pass it was acquired for is exactly the reentrant-lock
        // deadlock this design exists to avoid.
        drop(app.world_mut().remove_non_send::<ConnectedRes>());
...
/// an item registered coming due. Wakes the frame task and polls it
/// immediately rather than waiting for a run loop turn — [`skylight::acquire`]
/// resolves within one poll whenever nothing else holds the connection, which
/// is the common case, so a pass still happens synchronously here in
/// everything but name. The rare exception is a pass that lands while the
/// capture pool holds the connection: this call returns having only polled as
/// far as that, and [`poll_frame_task`]'s own doc says how it finishes. Doing
/// nothing when the app is absent matters: the waker is installed before the
/// app is built, so it can fire first.
///
/// Reentrancy is checked before anything else, not left to [`poll_frame_task`]
/// alone: the coalescing drain inside a pass already in progress can
/// re-dispatch the very run loop source this callback is running as, and a
/// notify queued for that reentrant call would still be sitting there when
/// the outer pass's loop comes back around to await the next one — one extra
/// pass for a signal the drain already accounted for.
pub fn pass() {
    ensure_frame_task();
    let free = FRAME_TASK.with(|slot| slot.try_borrow().is_ok());
    if !free {
        return;
    }
    FRAME.notify_one();
...
fn rebuild_panels(
    ...
    main: NonSend<Main>,
    connected: NonSend<ConnectedRes>,
) {
    // Waking counts as a display change here, because from the bar's side it is
    // indistinguishable from one: the machine may have slept with a display
    // attached and woken without it, and the window server is free to have
    // reconfigured the panels underneath us either way. Nothing else reacted to
    // a wake at all before this, which is why the bar could come back from
    // sleep and simply stop.
    if !events.read().any(|EventMessage(event)| {
        Kind::DisplayChanged.matches(event) || Kind::SystemWoke.matches(event)
    }) {
        return;
    }
    if let Err(err) = panels.rebuild(main.0, &connected.0, &settings) {
        tracing::error!(%err, "could not rebuild the bar after a display change or a wake");
    }
...
/// [`crate::protocol::wire::Reply`] once the walk finishes.
///
...
fn defer(
    request: &Request,
    reply: Option<crate::protocol::wire::Reply>,
) -> std::result::Result<(), Option<crate::protocol::wire::Reply>> {
    match *request {
...
fn routines(
    ...
    let now = Instant::now();
    let routine = std::sync::Arc::new(Event::Routine(crate::protocol::event::Routine {}));
...
mod idle_tests {
    ...
    use crate::protocol::{ItemName, Position};
