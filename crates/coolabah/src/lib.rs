//! The coolabah daemon: bar geometry, the item world, drawing, and the app that
//! drives them.
//!
...
//! |---|---|---|
//! | `coolabah-ax` ([`extras::Scanner`]) | 16, one task per pid | Accessibility RPCs into every running application |
//! | `coolabah-capture` ([`alias::Captor`]) | 4 | window pictures, pixel hashing, finding the inked rect |
//!
...
//!
//! - `coolabah-signals` ([`signals`]) — `sigwait`;
//! - `coolabah-config` ([`sources::config`]) — runs the config, which is a shell
//!   script that talks back to this daemon over IPC;
//! - `coolabah-script-N` ([`script`]) — subprocesses;
//! - `coolabah-subscriber-N` ([`subscribers`]) — one blocking Mach send per
