...
//! `--query` JSON shape and spellings, so a config parsing coolabah's output
...
//! the string `"on"`/`"off"`, never a JSON literal, and [`crate::protocol::Position`]
...
mod tests {
    ...
    fn an_item_state_renders_geometry_and_style() {
        let state = ItemState {
            ...
            percentage: 0,
            knob: Run::default(),
            highlight_color: crate::protocol::Color::default(),
        };
