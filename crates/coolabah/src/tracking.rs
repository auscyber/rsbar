...
//! Despawning is half the tidying; the other half is [`place`] itself,
//! removing the claim of anything that stopped being placed without being
//! despawned — an item whose `drawing` turned off, say.
//!
...
/// out, and releases the claim of anything that no longer does — including an
/// item that stopped being placed at all without being despawned, such as one
/// whose `drawing` turned off. That item is not reachable through
/// `placements` any more, so it is found the other way, through
/// [`TrackedArea`] itself, and dropped from the set the loop below builds up.
pub fn place(
    ...
    tracked: Query<Entity, With<TrackedArea>>,
) {
    let mut still_placed: HashSet<Entity> = HashSet::new();
    for placed in placements.panels() {
        for (entity, rect) in &placed.items {
            still_placed.insert(*entity);
    ...
    for entity in &tracked {
        if !still_placed.contains(&entity) {
            commands.entity(entity).remove::<TrackedArea>();
        }
    }
}
...
pub fn rebuild(
    ...
    {
        if let Err(err) = panel.window.clear_tracking_rects(&connected.0) {
            tracing::debug!(%err, "could not clear a display's tracked areas");
            continue;
        }
        let mut watched = 0;
        for rect in areas.on(panel.display.id) {
            match panel.window.add_tracking_rect(&connected.0, rect) {
                Ok(()) => watched += 1,
