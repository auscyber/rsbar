...
//! thread and nothing blocked on `mach_msg` anywhere. See [`coolabah::ipc`].
//!
...
//! that insists on a run loop of its own. See [`coolabah::sources`].
...
/// As in the library: module code names the wire vocabulary, never the product.
use ::coolabah_protocol as protocol;

use coolabah::bar::{Panels, Settings};
use coolabah::ecs::{self, Inbox};
use coolabah::script::Runner;
use coolabah::sources::Registry;
use protocol::wire::{MessagePack, Receiver};
use protocol::{Request, service_name};
use std::rc::Rc;
...
fn main() -> std::process::ExitCode {
    ...
    if !args.is_empty() {
        return coolabah::cli::run(args.into_iter());
    }
    ...
    let _stop = match coolabah::signals::Stop::listen(coolabah::ecs::exit_waker()) {
        Ok(stop) => stop,
        Err(err) => {
    ...
    let lock = match coolabah::lock::only(mtm, &service) {
        Ok(lock) => lock,
        Err(err) => {
    ...
    // Blocking, not awaited: this is before the run loop is entered, so
    // nothing is composited yet for a wait to stall -- and nothing else
    // exists yet to be holding the connection this first acquire wants.
    let connected = coolabah::pool::handle().block_on(skylight::acquire());
    if let Err(err) = panels.rebuild(&connected, &settings) {
    ...
    let waker = coolabah::ecs::waker();

    let config = coolabah::config::shared();
    let mut registry = Registry::new(coolabah::config::Shared::clone(&config), waker.clone());
    ...
    let requests = Rc::new(coolabah::ipc::Requests::default());
    ...
    let Some(_service) =
        coolabah::ipc::serve(receiver, Rc::clone(&requests), move || signal.wake())
    else {
        tracing::error!(%service, "could not put the service port on the run loop");
        return std::process::ExitCode::FAILURE;
    };

    tracing::info!(service = %service, "coolabah is up");
    let app = ecs::build(
        Inbox(requests),
        settings,
        panels,
...
fn init_tracing() {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_env(name!(env "LOG")).unwrap_or_else(|_| EnvFilter::new("info")),
        )
        .with_target(true)
        .init();
}
...
fn ask_for_screen_recording() {
    if coolabah::alias::screen_capture_trusted() {
        return;
    }
    ...
    let _ = coolabah::alias::request_screen_capture();
    ...
    tracing::error!(
        "still no Screen Recording permission; starting anyway. Aliases will \
         draw nothing until it is granted and coolabah is restarted."
    );
}
