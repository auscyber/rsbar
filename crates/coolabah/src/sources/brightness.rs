...
use crate::protocol::event::BrightnessChange;
...
///
/// Null while nothing is registered.
///
/// The registry drops the previous source's `Task` before asking for another,
/// but that drop is an *abort request*, not a synchronous stop: the old
/// registration's teardown can still run after the new one has stored its
/// context here. So the teardown clears this only if it is still its own,
/// through a compare-and-swap -- an unconditional store would blank a live
/// registration and leave brightness silent until the process restarted.
            ...
            let watch = match Callback::new(relay, |context| {
                ...
                match display.watch_brightness(changed) {
                    Ok(()) => Ok(move || {
                        ...
                        // Cleared only while it is still ours. Dropping a
                        // `Task` aborts, which is a request -- the old
                        // registration's teardown can therefore run *after* a
                        // new one has stored its own context, and an
                        // unconditional store would blank the live one and
                        // leave brightness silent for the rest of the process.
                        // Compare-and-swap makes a late teardown a no-op
                        // instead. A stray callback still finds null.
                        let _ = CONTEXT.compare_exchange(
                            context,
                            std::ptr::null_mut(),
                            Ordering::AcqRel,
                            Ordering::Acquire,
