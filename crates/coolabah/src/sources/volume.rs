...
use crate::protocol::event::VolumeChange;
...
fn relisten(device: &AtomicU32, context: *mut c_void) {
    ...
    for (selector, element) in WATCHED {
        // Said out loud rather than swallowed. Plenty of devices lack a
        // software volume control, so one of these failing is not fatal and
        // must not take the source down -- but a device that refuses all four
        // reports no volume change for as long as it is selected, and silence
        // is the one thing that looks identical to "nothing happened".
        let status = volume_listener(new_device, selector, element).add(context);
        if status != NO_ERROR {
            tracing::debug!(
                device = new_device,
                selector,
                element,
                status,
                "a volume property would not be listened to"
            );
        }
    }
