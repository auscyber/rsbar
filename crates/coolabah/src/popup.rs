...
//!
//! `position = "popup.<name>"` parses into [`crate::protocol::Position::Popup`],
...
pub use crate::protocol::{InvalidAlign, PopupAlign};
...
/// carries [`crate::protocol::Position::Popup`]; see the module doc.
///
...
/// available in this crate — it is a dependency of `coolabah-protocol`, not of
/// `coolabah` — so [`patched`] merges by hand rather than through that macro.
...
pub struct PopupPatch {
    ...
    pub background: Option<crate::protocol::BackgroundPatch>,
}
...
fn apply_background_patch(
    ...
    patch: &crate::protocol::BackgroundPatch,
...
impl Popups {
    ...
    fn ensure(
        ...
        if let std::collections::hash_map::Entry::Vacant(entry) = self.open.entry(host) {
            ...
            window.set_scale(connected, scale)?;
            window.set_opaque(connected, false)?;
            window.set_alpha(connected, 1.0)?;
            window.set_level(connected, config.level())?;
            window.set_tags(connected, WindowTags::BAR | WindowTags::OPAQUE_FOR_EVENTS)?;
...
pub fn repaint_popups(
    ...
    connected: NonSend<crate::ecs::ConnectedRes>,
    items: ItemQuery,
    ...
    for (host, config) in &hosts {
        ...
        let window = match popups.ensure(main.0, &connected.0, host, frame, scale, config) {
            Ok(window) => window,
            Err(err) => {
        ...
        if !just_opened
            ...
            && let Err(err) = window.set_frame(&connected.0, frame)
        {
            tracing::error!(?err, "could not reframe a popup window");
        }

        if !unchanged {
            skylight::draw_damaged(&connected.0, main.0, window, size, torn.as_deref(), |ctx| {
                fill_rounded_rect(
                    ctx,
...
#[cfg(test)]
mod visual_probe {
    //! Not part of the suite `cargo test -p coolabah` runs — like
    ...
    //! with `cargo test -p coolabah --lib popup::visual_probe -- --ignored
    ...
    fn a_popup_window_actually_renders() {
        ...
        let main = crate::runloop::main_thread();
        let window = skylight::Window::new(CGRect::new(CGPoint::new(80.0, 40.0), size), main)
        ...
        rt.block_on(async {
            ...
            window.set_opaque(&connected, false).unwrap();
            window.set_alpha(&connected, 1.0).unwrap();
            window
                .set_level(&connected, skylight::level::POPUP_MENU)
                .unwrap();
            window
                .set_tags(
                    &connected,
                    skylight::WindowTags::BAR | skylight::WindowTags::OPAQUE_FOR_EVENTS,
                )
                .unwrap();

            skylight::draw::<()>(&connected, main, &window, size, |ctx| {
            ...
            window.order_above(&connected, None).unwrap();
        });
        eprintln!(
