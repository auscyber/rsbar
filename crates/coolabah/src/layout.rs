...
pub fn arrange<T: Copy>(
    ...
    // which would overlap any centre item; coolabah hangs them off the centre
...
fn brackets_over(
    ...
    let mut frames: HashMap<&crate::protocol::ItemName, CGRect> =
...
#[allow(
    ...
    reason = "one system doing one whole surface's layout-and-draw pass, matching crate::popup::repaint_popups's own shape"
)]
pub fn repaint(
    items: ItemQuery,
    dirty: DirtyItems,
...
#[allow(
    clippy::too_many_arguments,
    reason = "the whole point is one pass over one surface; splitting it hides that"
)]
fn paint_panels(
    items: &ItemQuery,
    cache: &Cache,
