...
use crate::protocol::{BarPatch, BarState, Edge};
...
impl Settings {
    ...
    /// necessary.
    ///
    /// Every arm asks [`Changes`](crate::protocol::Changes) what the patch
    ...
    pub fn apply(&mut self, patch: &BarPatch) -> Changes {
        use crate::protocol::{Boolish, Changes as _};
...
impl Panels {
        ...
        for (index, display) in display::active(proof.marker()).into_iter().enumerate() {
            ...
            let panel = match self.panels.iter().position(|p| p.display.id == display.id) {
                Some(index) => {
                    ...
                    panel.window.set_frame(connected, frame)?;
                    if (panel.display.scale - display.scale).abs() > f64::EPSILON {
                        panel.window.set_scale(connected, display.scale)?;
                    }
    ...
    ///
    /// One batch, so a height change lands on every display at once rather
    /// than rippling across monitors. [`skylight::batched_with`] rather than
    /// [`skylight::batched`]: `connected` is already this frame's, and
    /// acquiring a second one here would deadlock against it.
    ///
    /// # Errors
    ///
    ...
    pub fn reframe(
        ...
        let frames: Vec<_> = self
            ...
            .collect();
        skylight::batched_with(connected, |connected| {
    ...
    pub fn set_blur(&self, connected: &skylight::Connected, radius: i32) -> skylight::Result<()> {
        for panel in &self.panels {
            panel.window.set_blur_radius(connected, radius)?;
        }
        Ok(())
    }
    ...
    pub fn set_level(
        ...
        for panel in &self.panels {
            panel.window.set_level(connected, bar_level(settings))?;
        }
        Ok(())
    }
    ...
    pub fn set_clickable(
        ...
        for panel in &self.panels {
            panel.window.clear_tags(connected, clear)?;
            panel.window.set_tags(connected, set)?;
        }
        Ok(())
    }
    ...
    pub fn set_sticky(
        ...
        for panel in &self.panels {
            panel.window.set_sticky(connected, sticky)?;
        }
        Ok(())
    }
    ...
    ///
    /// Returns the window server's error if a window rejects the change.
    pub fn set_show_in_fullscreen(
        &self,
        connected: &skylight::Connected,
        show: bool,
    ) -> skylight::Result<()> {
        for panel in &self.panels {
            panel.window.set_friend_of_fullscreen(connected, show)?;
        }
        Ok(())
    }
    ...
    pub fn set_hidden(
        ...
        for panel in &self.panels {
            if hidden {
                panel.window.order_out(connected)?;
            } else {
                panel.window.order_above(connected, None)?;
            }
        }
        Ok(())
    }
...
fn new_window(
    ...
    // Only `Window::new` needs it -- every mutation on the window it returns
    // takes `connected` instead, the same as everywhere else.
    let window = Window::new(frame)?;
    window.set_scale(connected, display.scale)?;
    window.set_opaque(connected, false)?;
    window.set_alpha(connected, 1.0)?;
    window.set_level(connected, bar_level(settings))?;
    let pointer = if clickable {
        WindowTags::OPAQUE_FOR_EVENTS
    } else {
        WindowTags::IGNORE_FOR_EVENTS
    };
    window.set_tags(connected, WindowTags::BAR | pointer)?;
    ...
    window.set_sticky(connected, settings.sticky)?;
    window.set_friend_of_fullscreen(connected, settings.show_in_fullscreen)?;
    if settings.blur_radius != 0 {
        window.set_blur_radius(connected, settings.blur_radius)?;
    }
    if !settings.hidden {
        window.order_above(connected, None)?;
    }
    Ok(window)
}
...
mod tests {
    ...
    use crate::protocol::{BarPatch, BoolChange, Boolish};
