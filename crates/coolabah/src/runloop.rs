...
impl Registration {
    /// Whether the deadline has passed and the wake already been delivered.
    ///
    /// [`wake_at`] takes the entry out of the reactor immediately *before*
    /// waking, so "no longer armed" is exactly "already fired".
    ///
    /// A caller that holds a registration to keep the wake alive needs this to
    /// tell one still coming from one already spent. Treating a spent
    /// registration as armed is a bar that never wakes again: the task behind
    /// it has run to completion and will not fire twice, so whoever is holding
    /// it has to arm a fresh one.
    #[must_use]
    pub fn fired(&self) -> bool {
        ARMED.with(|armed| !armed.borrow().contains_key(&self.id))
    }
}
...
mod pumped_tests {
    ...
    fn a_background_loop_runs_what_is_scheduled_on_it() {
        ...
        let pumped = Loop::start("coolabah-test-loop");
    ...
    fn a_background_loop_polls_a_future_built_on_its_own_thread() {
        let pumped = Loop::start("coolabah-test-spawn");
    ...
    fn a_pool_task_interleaves_with_a_loop_without_either_blocking() {
        ...
        let pumped = std::sync::Arc::new(Loop::start("coolabah-test-call"));
    ...
    fn a_value_crosses_all_three_executors_and_comes_home() {
        ...
        let sources = Loop::start("coolabah-test-round-trip");
    ...
    fn an_errand_runs_on_the_loop_that_was_given_it() {
        let pumped = Loop::start("coolabah-test-errand");
        let (tx, rx) = std::sync::mpsc::channel();
        ...
        assert_eq!(
            rx.recv_timeout(std::time::Duration::from_secs(5)),
            Ok(Some("coolabah-test-errand".to_owned())),
