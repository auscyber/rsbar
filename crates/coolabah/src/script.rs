...
//! # `sbar.exec`
//!
//! There is no `Request::Exec` here on purpose. `coolabah-lua`'s `exec_fn`
...
///
/// A config and its plugin scripts talk back by running `coolabah`, so that
...
/// generated wrapper, no second binary, just the real `coolabah` findable under
/// its real name.
///
/// Front rather than back, so a stale `coolabah` installed elsewhere cannot
...
async fn run(job: &Job) {
    ...
    let mut child = match command
        ...
        .envs(job.event.env())
        .env(name!(env "SERVICE"), crate::protocol::service_name())
        .env("PATH", child_path())
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::piped())
...
mod tests {
    ...
    fn a_plain_command_needs_no_shell() {
        assert_eq!(
            direct_argv("coolabah item set clock"),
            Some(vec!["coolabah", "item", "set", "clock"])
        );
    }
